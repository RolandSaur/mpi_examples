...
# Replace the print command with a communication command to the parent
#print("It took us %d iterations but we aggreed on %d" %
#      (iteration, TheNumber))

# get the parent communication channel
comm_parent = MPI.Comm.Get_parent()

# Have one of them send the result back to the parent process
if rank == 0:
    comm_parent.send((iteration, TheNumber), dest=0)
